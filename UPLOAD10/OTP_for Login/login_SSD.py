...
import sqlite3,time,math,random,smtplib,hashlib,sys  
    ...
    #Setup loop, gives user 3 try using for loop
        ...
        if results:
            ...
            #After 3 attempt print incorrect message before exiting the system
                ...
                print('Welcome to ASMIS '+results[3]+', you are now logged in') 
...
#Created password validation as function to prevent code duplication
    password = input("Please enter your password: ")
    password1 = input("Please enter your password: ")
    while password !=password1:
    #Validation, to match the password entered
        print("your password didnt match")
        #print the message when password not matched
        password = input("Please enter your password: ")
        password1 = input("Please enter your password: ")
    hashedpassword = hashlib.sha256(password.encode()).hexdigest()
    #For security, convert the password to hash using 
    #the hashlib.sha256, as its most secured hash
...
def passwordreset3():
        ...
        #Using the connection command to connect to the database called UserData as db
            ...
            print(results1)
        #updateData = ("UPDATE user SET password = ? WHERE username = ? VALUES (?,?)")
        #updateData = ("UPDATE user SET password = ? WHERE username = ? VALUSE (?)")
        #cursor.execute(updateData,[(username),(hashedpassword1)])
        #UPDATE user SET password=hashedpassword1 WHERE username=username
